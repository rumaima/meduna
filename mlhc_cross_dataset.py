...
def train_mlhc(args, model, tr_loader, val_loader, label_map, dataset_name, model_f):
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate((tr_loader)):
            ...
            Z = model.forward_normal_no_prompts(input_image_batch)
            Y = model_t(Z)

            optimizer.zero_grad()
            
            loss = - torch.log(torch.max(Y, axis=1)[0]).mean()
        ...
        # Now evaluate using the trained text classifier and tuned cross-modal prompts
        print(f'Evaluation: {epoch}')
        acc = evalauation_mlhc_mlp(val_loader, model, model_f, model_t, label_map)
        acc = np.round(acc,2)
        ...
        #         "epoch": epoch,
        #     }
        #     torch.save(checkpoint, os.path.join(model_save_path, "mlhc_cross_dataset_model_best.pth"))
...
def main(args):
    ...
    model_clip, preprocess, _ = clip.load("ViT-B/32", device=model.device)
    ...
    model_f = medclip_model.cpu()

    if args.zero_shot:
        zero_shot(model, test_loader)
    else:
        print(f'Dataset:{dataset_name}')
        train_mlhc(args, model, train_loader, test_loader, label_mapping, dataset_name, model_f)
