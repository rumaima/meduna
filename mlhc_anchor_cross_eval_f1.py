...
def evaluate_other_datasets_f1(dataloader, model, set, pickle_z=None):
    state_dict = torch.load(args.model_path)
    model.load_state_dict(state_dict["model"])
    model = model.cuda()
    model.eval()
    ...
    if set=='train':
        ...
        # N = N + N_train
        # pickle_dict = {'Ytrue_tr' : np.zeros((N_train,)), 
        #            'Yhat_tr' :np.zeros((N_train,))}
        
        pickle_dict['Ytrue_tr'] = np.zeros((N_train,))
        pickle_dict['Yhat_tr'] = np.zeros((N_train,))


    if set == 'val':
        N_val = len(dataloader.dataset)
        # N = N + N_val
        pickle_dict['Ytrue_va'] = np.zeros((N_val,))
        pickle_dict['Yhat_va'] = np.zeros((N_val,))
            
    if set == 'test':
        N_test = len(dataloader.dataset)
        # N = N + N_test
        pickle_dict['Ytrue_te'] = np.zeros((N_test,))
        pickle_dict['Yhat_te'] = np.zeros((N_test,))
    ...
    for i, inputs in enumerate(tqdm(dataloader)):
        ...
        with torch.no_grad():
            ...
            if set == 'train':
                pickle_dict['Yhat_tr'][pickle_dict['count']: pickle_dict['count']+len(inputs)] = Y_arg.cpu()
                pickle_dict['Ytrue_tr'][pickle_dict['count']: pickle_dict['count']+len(inputs)] = labels.cpu()
                pickle_dict['count'] += len(inputs)
            
            if set == 'val':
                pickle_dict['Yhat_va'][pickle_dict['count']: pickle_dict['count']+len(inputs)] = Y_arg.cpu()
                pickle_dict['Ytrue_va'][pickle_dict['count']: pickle_dict['count']+len(inputs)] = labels.cpu()
                pickle_dict['count'] += len(inputs)

            if set == 'test':
                pickle_dict['Yhat_te'][pickle_dict['count']: pickle_dict['count']+len(inputs)] = Y_arg.cpu()
                pickle_dict['Ytrue_te'][pickle_dict['count']: pickle_dict['count']+len(inputs)] = labels.cpu()
                pickle_dict['count'] += len(inputs)
...
def main(args):
    ...
    if args.zero_shot:
        ...
        other_train_acc = evaluate_other_datasets_f1(train_loader, model, set='train')
        other_val_acc = evaluate_other_datasets_f1(val_loader, model, set='val')
        other_test_acc = evaluate_other_datasets_f1(test_loader, model, set='test')

        pickle_z = pickle_z=args.pickle_file_path
        with open(pickle_z, 'wb') as f:
            pickle.dump(pickle_dict, f)
